...
For all land boxes RFR scenario
...
fn_Hg0 = '../GEOS-Chem_runs/run0314/OutputDir/GEOSChem.SpeciesConc.alltime_m.nc4'
ds_Hg0 = xr.open_dataset(fn_Hg0)
Hg0_conc = ds_Hg0.SpeciesConc_Hg0.isel(lev=0, time=slice(12, 24)).values # Hg0 in mole/mole
lat = ds_Hg0.lat.values # latitude
lon = ds_Hg0.lon.values # longitude
lat_l = len(lat)
lon_l = len(lon)
...
for pp in range(num): # loop over parameters
    ...
    # Land type areas - REF
    fn_ols2 = 'misc_Data/RFR/unc_LAI/Olson_Land_Type_Masks.2x25.RFR_unc_' + str(pp) + '.nc'
    ...
    fn_xlai  = 'misc_Data/RFR/unc_LAI/Yuan_proc_MODIS_XLAI.2x25.RFR_unc_' + str(pp) + '_m.nc'
...
np.savetxt("misc_Data/unc_dep_RFR_global_all_land.csv", land_dep, delimiter=",")
# save data to mat, for easier access
sio.savemat('misc_Data/unc_drydepflux_m_RFR_all_land.mat', {"DF": DF_ann_u})
