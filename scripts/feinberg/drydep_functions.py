...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 27 09:29:55 2021
...
def DEPVEL_FOREST(DRYCOEFF, IOLSON, IDEP, IRI, IRLU, IRAC, IRGSS, IRGSO, IRCLS, IRCLO, \
           IREG, ILAND, IUSE, \
           TS, XLAI, LSNOW, RADIAT, CLDFRC, SUNCOSMID, PRESSU, USTAR, Z0, \
           CZ1, OBK, XMW, F0, HSTAR):
    """ Calculate the deposition velocity for Hg0 - 1D version for grid cell
    ...
    for LDT in range(IREG):
        ...
        if (not ((II == 2) | (II ==3))): # only calculate within forest area
            continue
        # Scale XLAI by the fraction of area of land type
        XLAI_scale = XLAI[IOLSON] / IUSE[LDT] * 1000.
...
def DEPVEL_AMAZON(DRYCOEFF, IOLSON, IDEP, IRI, IRLU, IRAC, IRGSS, IRGSO, IRCLS, IRCLO, \
           IREG, ILAND, IUSE, \
           TS, XLAI, LSNOW, RADIAT, CLDFRC, SUNCOSMID, PRESSU, USTAR, Z0, \
           CZ1, OBK, XMW, F0, F0_AMZN, in_AM, HSTAR):
    """ Calculate the deposition velocity for Hg0 - 1D version for grid cell - allowing for 2 F0 values
    ...
    F0 : float
         biological reactivity of species
    F0_AMZN : float
         biological reactivity of species in Amazon rainforest
    in_AM : boolean
         whether inside South American domain or not
    HSTAR : float
         Henry's law constant at pH 7 (for surface)
    """
    ...
    for LDT in range(IREG):
        ...
        IOLSON = ILAND[LDT] # Olson land type (0 - 72)
        II = IDEP[IOLSON] # dry deposition land type (1 - 11)
        if (II == 6 and in_AM == True): # rainforest
            F0_use = F0_AMZN 
        else:
            F0_use = F0
        # Scale XLAI by the fraction of area of land type
        ...
        RIXX = RIX * DIFFG(TEMPK, PRESSU, XMWH2O) / \
            DIFFG(TEMPK, PRESSU, XMW) + \
            1 / (HSTAR / 3000. + 100. * F0_use)
        
        # cuticular resistance
        RLUXX = 1.e12
        if (RLU[LDT] < 9999.):
             RLUXX = RLU[LDT] / (HSTAR / 1e5 + F0_use)
        
        # soil resistance
        RGSX = 1. / (HSTAR / 1e5 / RGSS[LDT] + F0_use / RGSO[LDT])
        
        # lower canopy resistance
        RCLX = 1. / (HSTAR / 1e5 / RCLS[LDT] + F0_use / RCLO[LDT])
...
def DEPVEL_UNC(DRYCOEFF, IOLSON, IDEP, IRI, IRLU, IRAC, IRGSS, IRGSO, IRCLS, IRCLO, \
           IREG, ILAND, IUSE, \
           TS, XLAI, LSNOW, RADIAT, CLDFRC, SUNCOSMID, PRESSU, USTAR, Z0, \
           CZ1, OBK, XMW, F0, F0_AMZN, F0_RF, in_AM, HSTAR):
    """ Calculate the deposition velocity for Hg0 - 1D version for grid cell - allowing for 3 F0 values
    ...
    F0_AMZN : float
         biological reactivity of species in Amazon rainforest
    F0_RF : float
         biological reactivity of species in other rainforests        
    in_AM : boolean
         whether inside South American domain or not
    HSTAR : float
         Henry's law constant at pH 7 (for surface)
